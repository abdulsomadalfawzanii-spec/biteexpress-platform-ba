...
/* Helper — sends the first validation error as a 400 response */
const checkValidation = (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({ success: false, message: errors.array()[0].msg });
    return true; // signals "already responded"
  }
  return false;
};
...
router.post('/register', registerValidation, async (req, res) => {
  try {
    ...
    // 'admin' is not a registerable role — silently drop to 'customer'
    ...
    const user = await User.create({
      ...
      role:                safeRole,
      address:             address || '',
      // Vendor accounts start as 'pending' until admin approval
      accountStatus:       safeRole === 'vendor' ? 'pending' : 'active',
    ...
    // Send verification email — non-blocking, never kills registration
...
router.post('/login', loginValidation, async (req, res) => {
  try {
    ...
    // Vendor-specific: warn if still pending but allow login so they can see their status
    if (user.role === 'vendor' && user.accountStatus === 'pending') {
      // Allow login — frontend will show appropriate pending state
    }
...
/* ─── POST /auth/admin/create ────────────────────────
   Secure admin creation — requires an existing admin JWT.
   Usage: existing admin calls this to create another admin.
   First admin is created via the seed script (npm run seed:admin).
──────────────────────────────────────────────────── */
